...
pub async fn handle_get(
    ...
    .await
    {
        Ok(value) => value,
        Err(error) => {
            tracing::error!("Failed to query {qname}/{}: {error:?}", rrtype);
            return Err(response_500()); // TODO: This should probably be a SERVFAIL
        }
    };

    tracing::trace!("Completed record request...");

    let ttl = records.iter().map(|r| r.ttl).min();
    let ttl = match ttl {
        Some(val) => val.to_owned(),
        None => {
            tracing::trace!("Failed to get minimum TTL from query, using 1");
            1
        }
    };

    tracing::trace!("Returned records: {records:?}");
    ...
    match response_type {
        ...
        ResponseType::Json => {
            ...
            let response = serde_json::to_string(&reply).map_err(|err| {
                tracing::error!("Failed to parse DoH GET request into JSON: {err:?}");
                response_500()
            })?;
            ...
            response_builder.body(Body::from(response)).map_err(|err| {
                tracing::error!("Failed to turn DoH GET request into JSON: {err:?}");
                response_500()
            })
        }
        ResponseType::Raw => {
            ...
            match reply.as_bytes().await {
                Ok(value) => axum::response::Response::builder()
                    ...
                    .map_err(|err| {
                        tracing::error!("Failed to turn DoH GET request into bytes: {err:?}");
                        response_500()
                    }),
                Err(err) => {
                    tracing::error!("Failed to turn DoH GET request into bytes: {err:?}");
    ...
    match res {
        Ok(mut reply) => {
            let bytes = match reply.as_bytes().await {
                Ok(value) => {
                    ...
                    if value.len() > 65535 {
                        ...
                        let mut bytes: Vec<u8> = reply.as_bytes().await.map_err(|err| {
                            tracing::error!("Failed to turn DoH POST response into bytes: {err:?}");
                            response_500()
                        })?;
                ...
                Err(error) => {
                    tracing::error!("Failed to turn DoH POST response into bytes! {error:?}");
        ...
        Err(err) => {
            tracing::error!("Failed to parse DoH POST query: {err:?}");
            Err(response_500())
        }
    }
}
