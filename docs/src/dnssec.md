...
GoatNS supports serving DNSSEC-signed zones. This is an authoritative-only server — it does not
perform DNSSEC validation (that is a recursive resolver concern). The deployment model is **offline
signing**: zones are signed by a companion tool before being loaded into the server.

## Overview

DNSSEC (Domain Name System Security Extensions) provides authentication and integrity for DNS data
using public-key cryptography. When a zone is signed:
...
Upload the DS records to your registrar or parent zone operator. This establishes the chain of
trust.
